        ...
        public bool ComposeResolvableSkippedCalls(PurityAnalysisData data)
        {
            ...
            do
            {
                ...
                foreach (var call in data.SkippedCalls)
                {
                    ...
                    //ignoring live threads
                    if (calltype.threadstart == true)
                    {
                        var recvrs = AnalysisUtil.GetReceiverVertices(call as StaticCall, data);
                        if (recvrs.Intersect(data.GetThreadObjs()).Any())
                            continue;
                    }
                    //ignoring unresolvable & thread start calls
