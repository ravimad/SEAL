...
namespace SafetyAnalysis.Purity
{
    ...
    {        
        ...
        {
            //if (functionUnit.FunctionSymbol.NameString.Equals("Run"))
            //{
            //}
        ...
        private void CleanupSummary(PurityAnalysisData data)
        {
            ...
            //this cannot be called if tracking read sets
        ...
        {
            var knownLinqMethods = (data.linqDelegates.OfType<MethodHeapVertex>()).ToList();
            MethodLevelAnalysis.MethodsPassedToLinqQueries.UnionWith(knownLinqMethods);
            data.linqDelegates.RemoveWhere((HeapVertexBase v) => (v is MethodHeapVertex));
        }        
    }      
}
