...
namespace SafetyAnalysis.Purity
{
    public class AnalysisUtil
    {
        ...
        public static IEnumerable<HeapVertexBase> GetReceiverVertices(StaticCall scall,
            PurityAnalysisData data)
        {
            if (scall.GetParamCount() <= 0)
                throw new NotSupportedException("No receiver for static call: " + scall);

            var recvr = scall.GetParam(0);
            ...
            PurityAnalysisData calleeData, List<object> extStr)
        {
            ...
            var transLinqMethods = from del in calleeData.linqDelegates                                  
                                   select nsMap[del];
            transData.linqDelegates = new HeapVertexSet(transLinqMethods);
