...
namespace SafetyAnalysis.Purity
{
    ...
    public class FlowSensitiveAnalysis
    {
        ...
        //private static StreamWriter threadStream = new StreamWriter(File.OpenWrite("./thread-constructs.txt"));
        //private HashSet<uint> processedInsts = new HashSet<uint>();
            ...
            PartitionPurityData inData)
        {
            ...
            foreach (Phx.IR.Instruction instruction in block.Instructions)
            {
                ...
                {
                    //if (instruction.AsCallInstruction.FunctionSymbol.QualifiedName.Equals(
                    //    "[mscorlib]System.Threading.Thread::.ctor"))
                    //{
                    //    Console.WriteLine(instruction);
                    //}
                    //var instid = AnalysisUtil.GetCallInstId(instruction.AsCallInstruction);
                    //if (!processedInsts.Contains(instid))
                    //{
                    //    processedInsts.Add(instid);

                    //    var calledfunc = PhxUtil.GetQualifiedFunctionName(instruction.AsCallInstruction.FunctionSymbol);
                    //    if (calledfunc.Contains("System.Threading"))
                    //    {

                    //        //added temporarily to output the thread constructs
                    //        threadStream.WriteLine(calledfunc + " calledBy: " +
                    //            PhxUtil.GetQualifiedFunctionName(instruction.FunctionUnit.FunctionSymbol));
                    //        threadStream.Flush();
                    //    }
                    //}
                }
