    ...
    public class Simplifier
    {
        ...
        public void RemoveCapturedOrUnresolvableCalls()
        {
            ...
            var enodes = from v in data.OutHeapGraph.Vertices
                         ...
                         select v;
            //also add the live thread objects to enodes
            enodes = enodes.Union(data.GetThreadObjs());
            ...
            do
            {
                ...
                foreach (var call in data.SkippedCalls)
                {
                    ...
                    {
                        methodname = (call as StaticCall).GetMethodName();
                        typename = (call as StaticCall).GetDeclaringType();

                        receivers.AddRange(AnalysisUtil.GetReceiverVertices(call as StaticCall, data));
                    }
                    else if (call is VirtualCall)
                    {
                        methodname = (call as VirtualCall).GetMethodName();
                        typename = (call as VirtualCall).GetDeclaringType();
                    ...
                    //check if the call is a thread create method and if the receiver object is live
                    bool escapingCall = false;
                    if ((call is StaticCall) 
                        && CallUtil.IsThreadCreator(typename,methodname)
                        && receivers.Intersect(data.GetThreadObjs()).Any())
                    {
                        escapingCall = true;
                    }
                    else
        ...
        {            
            ...
            foreach (var v in data.OutHeapGraph.Vertices)
            {
                ...
                foreach (var e in data.OutHeapGraph.OutEdges(v))
                {
                    ...
                    //Conservatively, do not remove fields defined by stubs
                    //this could be made precise later if needed
                    if ((e.Field is NamedField) &&
                        (e.Field as NamedField).Name.StartsWith(PurityAnalysisPhase.stubFieldPrefix))
                        continue;
