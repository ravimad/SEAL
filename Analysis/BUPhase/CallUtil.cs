...
namespace SafetyAnalysis.Purity
{
    public class CallUtil
    {
        public static CallType GetCallType(Call call, PurityAnalysisData data)
        {
            ...
            //set the thread creator and join fields
            if (call is CallWithMethodName)
            {
                var scall = call as CallWithMethodName;
                if (IsThreadCreator(scall.GetDeclaringType(), scall.GetMethodName()))
                    calltype.threadstart = true;
                else if (IsThreadJoin(scall.GetDeclaringType(), scall.GetMethodName()))
                    calltype.threadjoin = true;
                else
                {
                    calltype.threadjoin = false;
                    calltype.threadstart = false;
                }
            }
        ...
        public static bool IsThreadCreator(string typename, string methodname)
        {
            if (typename.Equals("[mscorlib]System.Threading.Thread") && methodname.Equals("Start"))
                return true;
            return false;
        }

        public static bool IsThreadJoin(string typename, string methodname)
        {
            if (typename.Equals("[mscorlib]System.Threading.Thread") && methodname.Equals("Join"))
                return true;
            return false;
        }
    }
