...
namespace SafetyAnalysis.Purity
{
    public class PurityDataUtil
    {
            ...
            HeapVertexBase rep)
        {
            ...
            if (data.linqDelegates.Any((HeapVertexBase b) => (toCollapse.Contains(b))))
            {
                data.linqDelegates.RemoveWhere((HeapVertexBase b) => (toCollapse.Contains(b)));
                data.linqDelegates.Add(rep);
            }
        }
